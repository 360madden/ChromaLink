...
public static class ChromaLinkWindowTools
{
...
function Get-WindowGeometry {
  ...
  $windowRect = New-Object ChromaLinkWindowTools+Rect
  if (-not [ChromaLinkWindowTools]::GetWindowRect($Handle, [ref]$windowRect)) {
    throw "GetWindowRect failed."
  }

  $clientRect = New-Object ChromaLinkWindowTools+Rect
  $clientPoint = New-Object ChromaLinkWindowTools+Point
  $hasClient = [ChromaLinkWindowTools]::GetClientRect($Handle, [ref]$clientRect) -and [ChromaLinkWindowTools]::ClientToScreen($Handle, [ref]$clientPoint)

  $style = [ChromaLinkWindowTools]::GetWindowLongPtr($Handle, [ChromaLinkWindowTools]::GwlStyle).ToInt64()
  $exStyle = [ChromaLinkWindowTools]::GetWindowLongPtr($Handle, [ChromaLinkWindowTools]::GwlExStyle).ToInt64()
  ...
  return [pscustomobject]@{
    ...
    IsMinimized = [ChromaLinkWindowTools]::IsIconic($Handle)
    Style = $style
    ExStyle = $exStyle
  }
}
...
function Normalize-ScreenCoordinate {
  ...
  $screenSize = [ChromaLinkWindowTools]::GetSystemMetrics($Metric)
...
function Get-OuterSize {
  ...
  $rect = New-Object ChromaLinkWindowTools+Rect
  $rect.Left = 0
  $rect.Top = 0
  $rect.Right = $RequestedClientWidth
  $rect.Bottom = $RequestedClientHeight

  if (-not [ChromaLinkWindowTools]::AdjustWindowRectEx([ref]$rect, [int]$Style, $false, [int]$ExStyle)) {
    return [pscustomobject]@{
...
if ($before.IsMinimized) {
  [void][ChromaLinkWindowTools]::ShowWindow($handle, [ChromaLinkWindowTools]::SwRestore)
  Start-Sleep -Milliseconds 200
}

$working = Get-WindowGeometry -Handle $handle
$targetLeft = if ($Left -ge 0) { $Left } else { Normalize-ScreenCoordinate -Coordinate $working.WindowLeft -Metric ([ChromaLinkWindowTools]::SmCxScreen) }
$targetTop = if ($Top -ge 0) { $Top } else { Normalize-ScreenCoordinate -Coordinate $working.WindowTop -Metric ([ChromaLinkWindowTools]::SmCyScreen) }
...
if (-not [ChromaLinkWindowTools]::SetWindowPos($handle, [System.IntPtr]::Zero, $targetLeft, $targetTop, $outerSize.Width, $outerSize.Height, [ChromaLinkWindowTools]::SwpNoZOrder -bor [ChromaLinkWindowTools]::SwpNoActivate)) {
...
if ($widthError -ne 0 -or $heightError -ne 0) {
  ...
  if (-not [ChromaLinkWindowTools]::SetWindowPos($handle, [System.IntPtr]::Zero, $targetLeft, $targetTop, $correctedWidth, $correctedHeight, [ChromaLinkWindowTools]::SwpNoZOrder -bor [ChromaLinkWindowTools]::SwpNoActivate)) {
